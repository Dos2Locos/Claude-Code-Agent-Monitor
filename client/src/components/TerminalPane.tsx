  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ws.onclose = (e) => {
        if (!disposed) {
          setConnected(false);
          if (e.code === 4404) setError("tmux session unavailable (it may have ended).");
          else if (e.code !== 1000) setError(`Connection closed (${e.code}).`);
        }
      };
      ...
      if (!disposed) setError(`Failed to initialize terminal: ${err.message}`);
    });
